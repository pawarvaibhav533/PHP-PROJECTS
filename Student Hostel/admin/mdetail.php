...
<table width="1023" height="400" border="1">
  <tbody>
    ...
    <tr>
	  ...
	  </div><h2><center>MESS DETAIL</h2></center>
	  <?php
...
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM `messdetail` WHERE CONCAT(`id`, `username`, `checkin_date`, `checkout_date`, `mess_type`, `no_of_seats`, `amount`) LIKE '%".$valueToSearch."%'";
    $search_result = filterTable($query);
    
}
 else {
    $query = "SELECT * FROM `messdetail`";
    $search_result = filterTable($query);
}
   ...
   <body>
      ...
      <!-- populate table from mysql database -->
                ...
                <tr>
					...
					<td><center><?php echo $row['no_of_seats'];?></center></td>
					<td><center><?php echo $row['amount'];?></center></td>
					<td><a href='http://localhost/phpmyadmin/sql.php?server=1&db=hostel&table=messdetail&pos=0'>Delete</a></td>
                </tr>
